...
if GetMapName() == "frostivus_2017" then
	require( "zones/zone_tables_frostivus_2017" )
end
...
function CDungeonZone:SpawnChests()
	...
	for index, chestTable in ipairs( self.Chests ) do
		...
		if ( chestTable.szSpawnerName ) then
...
function CDungeonZone:SpawnBreakables()
	...
	for index, breakableTable in ipairs( self.Breakables ) do
		...
		if ( breakableTable.szSpawnerName ) then
			...
			if ( vip.nMaxSpawnCount == nil ) or ( vip.nSpawnAmt < vip.nMaxSpawnCount ) then
				for i=1,vip.nCount do
					...
					if hUnit then
						...
						if hAnimationBuff and vip.Activity then
			...
			if ( neutral.nMaxSpawnCount == nil ) or ( neutral.nSpawnAmt < neutral.nMaxSpawnCount ) then
				for i=1,neutral.nCount do
					...
					if hUnit then
						...
						hUnit:AddNewModifier( hUnit, nil, "modifier_npc_dialog", { duration = -1 } )
						local hAnimationBuff = hUnit:AddNewModifier( hUnit, nil, "modifier_stack_count_animation_controller", {} )
						if hAnimationBuff and neutral.Activity then
				...
				if hUnit then
					...
					for i=1,#Heroes do 
						if hAttackTarget == nil then
							...
							if hAttackTarget and self:ContainsUnit( hAttackTarget ) and ( hAttackTarget:IsRealHero() == false or hAttackTarget:IsAlive() == false ) then
								hAttackTarget = nil
							end 
						end
					end
			...
			if hWaypoint == nil or hSpawner == nil then
				if hWaypoint == nil then
					for _,VIP in pairs( self.VIPsAlive ) do
						if VIP and VIP:GetUnitName() == npc.szWaypointName then
							hWaypoint = VIP
						end
					end
...
function CDungeonZone:GetRandomHero()
	...
	for _, hHero in pairs( hAllHeroes ) do
		if hHero and ( hHero:GetTeamNumber() == DOTA_TEAM_GOODGUYS ) and hHero:IsRealHero() and hHero:IsAlive() and hHero:IsInvulnerable() == false and hHero:IsOutOfGame() == false then
			return hHero
		end
	end
end
...
function CDungeonZone:OnZoneActivated( zone )
	for _,quest in pairs ( self.Quests ) do
		if quest and quest.bCompleted == false then
			if quest.bActivated == false then
				local bShouldActivate = false
				for _,activator in pairs( quest.Activators ) do
					if activator and activator.Type == QUEST_EVENT_ON_ZONE_ACTIVATE and activator.szZoneName == zone.szName then
					...
					if activator.Type == QUEST_EVENT_ON_DIALOG or activator.Type == QUEST_EVENT_ON_DIALOG_ALL_CONFIRMED and zone.szName == self.szName then
						...
						for _,DialogEnt in pairs ( hDialogEntities ) do
							if DialogEnt  and DialogEnt:GetUnitName() == activator.szNPCName and DialogEnt:FindModifierByName( "modifier_npc_dialog_notify" ) == nil then
...
function CDungeonZone:OnZoneEventComplete( zone )
	for _,quest in pairs ( self.Quests ) do
		if quest and not quest.bCompleted == true then
			if quest.bActivated == false then
				local bShouldActivate = false
				for _,activator in pairs( quest.Activators ) do
					if activator and activator.Type == QUEST_EVENT_ON_ZONE_EVENT_FINISHED and activator.szZoneName == zone.szName then
						bShouldActivate = true
					end
				end
				if bShouldActivate == true then
	...
	for _,quest in pairs ( self.Quests ) do
		if quest and not quest.bCompleted == true then
			if quest.bActivated == false then
				local bShouldActivate = false
				for _,activator in pairs( quest.Activators ) do
					if activator and activator.Type == QUEST_EVENT_ON_CUSTOM_EVENT and activator.szEventName == szEventName and activator.szZoneName == szZoneName then
						bShouldActivate = true
					end
				end
				if bShouldActivate == true then
	...
	for _,quest in pairs ( self.Quests ) do
		if quest and not quest.bCompleted == true then
			if quest.bActivated == false then
				local bShouldActivate = false
				for _,activator in pairs( quest.Activators ) do
					if activator and activator.Type == QUEST_EVENT_ON_ENEMY_KILLED and activator.szNPCName == hDeadEnemy:GetUnitName() and ( ( quest.Completion.szZoneName == Zone.szName ) or ( quest.Completion.szZoneName == nil ) ) then
						bShouldActivate = true
					end
				end
	...
	for _,quest in pairs ( self.Quests ) do
		if quest and not quest.bCompleted == true then
			if quest.bActivated == false then
				local bShouldActivate = false
				for _,activator in pairs( quest.Activators ) do
					if activator and activator.Type == QUEST_EVENT_ON_TREASURE_OPENED and activator.szZoneName == szZoneName then
						bShouldActivate = true
					end
				end
	...
	for _,quest in pairs ( self.Quests ) do
		if quest and not quest.bCompleted == true then
			if quest.bActivated == false then
				local bShouldActivate = false
				for _,activator in pairs( quest.Activators ) do
					if activator and activator.Type == QUEST_EVENT_ON_KEY_ITEM_RECEIVED and activator.szItemName == szItemName and activator.szZoneName == szZoneName then
						bShouldActivate = true
					end
				end
	...
	for _,quest in pairs ( self.Quests ) do
		if quest and not quest.bCompleted == true then
			if quest.bActivated == false then
				local bShouldActivate = false
				for _,activator in pairs( quest.Activators ) do
					if activator and activator.Type == QUEST_EVENT_ON_DIALOG and activator.szNPCName == hDialogEnt:GetUnitName() and hDialogEnt.nCurrentLine == activator.nDialogLine then
						bShouldActivate = true
					end
				end
	...
	for _,quest in pairs ( self.Quests ) do
		if quest and not quest.bCompleted == true then
			if quest.bActivated == false then
				local bShouldActivate = false
				for _,activator in pairs( quest.Activators ) do
					if activator and activator.Type == QUEST_EVENT_ON_DIALOG_ALL_CONFIRMED and hDialogEnt and activator.szNPCName == hDialogEnt:GetUnitName() and nDialogLine == activator.nDialogLine then
						bShouldActivate = true
					end
				end
			...
			if quest.bActivated == true and quest.Completion.Type == QUEST_EVENT_ON_DIALOG_ALL_CONFIRMED and hDialogEnt and quest.Completion.szNPCName == hDialogEnt:GetUnitName() and nDialogLine == quest.Completion.nDialogLine then
				GameRules.Dungeon:OnQuestCompleted( self, quest ) 
			end
		end
	end
...
function CDungeonZone:StartQuestByName( szQuestName )
	for _,quest in pairs ( self.Quests ) do
		if quest and quest.szQuestName == szQuestName then
...
function CDungeonZone:OnQuestStarted( quest )
	for _,zoneQuest in pairs ( self.Quests ) do
		if zoneQuest and not zoneQuest.bCompleted == true then
			if zoneQuest.bActivated == false then
				local bShouldActivate = false
				for _,activator in pairs( zoneQuest.Activators ) do
					if activator and activator.Type == QUEST_EVENT_ON_QUEST_ACTIVATE and activator.szQuestName == quest.szQuestName then
						bShouldActivate = true
					end
				end
	...
	for _, vip in pairs( self.VIPsAlive ) do
		...
		if Dialog and Dialog.szAdvanceQuestActive == quest.szQuestName then
	...
	if self.Type == ZONE_TYPE_SURVIVAL then
		if self.Survival.StartQuest and self.Survival.StartQuest.bOnCompleted == false then
	...
	if self.Type == ZONE_TYPE_HOLDOUT then
		if self.Holdout.StartQuest and self.Holdout.StartQuest.bOnCompleted == false then
	...
	if self.Type == ZONE_TYPE_ASSAULT then
		if self.Assault.StartQuest and self.Assault.StartQuest.bOnCompleted == false then
...
function CDungeonZone:OnQuestCompleted( quest )
	for _,zoneQuest in pairs ( self.Quests ) do
		if zoneQuest and not zoneQuest.bCompleted == true then
			if zoneQuest.bActivated == false then
				local bShouldActivate = false
				for _,activator in pairs( zoneQuest.Activators ) do
					if activator and activator.Type == QUEST_EVENT_ON_QUEST_COMPLETE and activator.szQuestName == quest.szQuestName then
						bShouldActivate = true
					end
				end
	...
	if self.Type == ZONE_TYPE_SURVIVAL then
		if self.Survival.StartQuest and self.Survival.StartQuest.bOnCompleted == true then
			if quest.szQuestName == self.Survival.StartQuest.szQuestName then
	...
	if self.Type == ZONE_TYPE_HOLDOUT then
		if self.Holdout.StartQuest and self.Holdout.StartQuest.bOnCompleted == true then
			if quest.szQuestName == self.Holdout.StartQuest.szQuestName then
...
function CDungeonZone:OnThink()
	...
	if self.bActivated == false then
		...
		for _,enemy in pairs ( self.Enemies ) do
			if enemy and not enemy:IsNull() and enemy:IsAlive() and enemy.bAttacker == true then
...
function CDungeonZone:CheckForZoneComplete()
	...
	elseif self.bZoneCompleted == false then
		...
		if not self.bNoLeaderboard then
			for nPlayerID = 0,3 do
				if self.PlayerStats[nPlayerID] then
					if self.PlayerStats[nPlayerID]["Kills"] then self.nKills = self.nKills + self.PlayerStats[nPlayerID]["Kills"] end
					if self.PlayerStats[nPlayerID]["Items"] then self.nItems = self.nItems + self.PlayerStats[nPlayerID]["Items"] end
					if self.PlayerStats[nPlayerID]["GoldBags"] then self.nGoldBags = self.nGoldBags + self.PlayerStats[nPlayerID]["GoldBags"] end
					if self.PlayerStats[nPlayerID]["Potions"] then self.nPotions = self.nPotions + self.PlayerStats[nPlayerID]["Potions"] end
					if self.PlayerStats[nPlayerID]["ReviveTime"] then self.nReviveTime = self.nReviveTime + self.PlayerStats[nPlayerID]["ReviveTime"] end
					if self.PlayerStats[nPlayerID]["Damage"] then self.nDamage = self.nDamage + self.PlayerStats[nPlayerID]["Damage"] end
					if self.PlayerStats[nPlayerID]["Healing"] then self.nHealing = self.nHealing + self.PlayerStats[nPlayerID]["Healing"] end
					if self.PlayerStats[nPlayerID]["Deaths"] then 
			...
			if self.StarCriteria then
					...
					if Criteria then
						...
						if Criteria.Type == ZONE_STAR_CRITERIA_TIME then
							...
							for i=1,#Criteria.Values do
								...
								if time and time >= Criteria.Result then 
						...
						if Criteria.Type == ZONE_STAR_CRITERIA_DEATHS then
							...
							for i=1,#Criteria.Values do
								...
								if deaths and deaths >= Criteria.Result then 
									if i > Criteria.StarScore then
										Criteria.StarScore = i
									end
								end
						...
						if Criteria.Type == ZONE_STAR_CRITERIA_QUEST_COMPLETE then
							...
							for i=1,#Criteria.Values do
								...
								if completed and completed <= Criteria.Result then 
									if i > Criteria.StarScore then
										Criteria.StarScore = i
									end
								end
				...
				self.nStars = 9999
				for _,Criteria in pairs( self.StarCriteria ) do
					if Criteria and Criteria.StarScore and Criteria.StarScore < self.nStars then
...
function CDungeonZone:PerformZoneCleanup()
	if #self.Enemies > 0 then
		...
		for i=#self.Enemies,1,-1 do
			...
			if enemy and enemy:IsNull() == false then
				local bCanBeSeen = false 
				for _,Hero in pairs( Heroes ) do
					if Hero:CanEntityBeSeenByMyTeam( enemy ) then
						bCanBeSeen = true
	...
	for _,Boss in pairs ( self.Bosses ) do
		if Boss.bAwake == nil or Boss.bAwake == true then
			...
			--print( "CDungeonZone:BossThink() - Boss has not started" )
				for _,Hero in pairs ( Heroes ) do
					if Hero and Hero:GetTeamNumber() == DOTA_TEAM_GOODGUYS and Hero:CanEntityBeSeenByMyTeam( Boss ) then
...
function CDungeonZone:HoldoutStart()
	...
	for _,neutral in pairs( self.NeutralsAlive ) do
		if neutral and neutral:IsNull() == false and neutral:FindAbilityByName( "ability_journal_note" ) == nil then
...
function CDungeonZone:AssaultStart()
	...
	if hRescuedAttackerEntity then
...
function CDungeonZone:AllQuestsComplete()
	for _,quest in pairs ( self.Quests ) do
		if quest and quest.bCompleted == false and quest.bOptional == false then
			return false
		end
	end
...
function CDungeonZone:RemoveItemDropsFromZoneEnemies()
	...
	for _,enemy in pairs ( self.Enemies ) do
		if enemy and enemy.bBoss == false then
