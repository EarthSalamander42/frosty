		...
		if hPlayerHero.bFirstSpawnComplete == nil then
			...
			if not IsDedicatedServer() and not PlayerResource:IsFakeClient( hPlayerHero:GetPlayerOwnerID() ) then
				...
				elseif GetMapName() == "ep_2" then
					...
					self:OnPlayerHeroEnteredZone( hPlayerHero, "tavern" )
					CustomGameEventManager:Send_ServerToPlayer( PlayerResource:GetPlayer( hPlayerHero:GetPlayerOwnerID() ), "hide_scroll", {} )
				end
			end
			...
			if self.bExplorerMode ~= true then
				...
				for _,Relic in pairs( self.RelicsDefinition ) do
					if Relic and GetItemDefOwnedCount( nPlayerID, Relic["DungeonItemDef"] ) > 0 then
...
function CDungeon:OnNPCSpawned_EnemyCreature( event )
	...
	if hEnemyCreature and self.bExplorerMode == true then
...
function CDungeon:OnEntityKilled_PlayerHero( event )
	...
	if killedUnit and killedUnit:IsRealHero() then
		...
		if OrbOfPassage and killedUnit.nRespawnsRemaining == 0 then
		...
		if hDeadCreature.bBoss == true and Zone:GetNumberOfBosses() == 0 then
			...
			if hDeadCreature:GetUnitName() == "npc_dota_creature_elder_tiny" then
				...
				for _, neutral in pairs ( neutrals ) do
					if neutral and ( neutral:GetUnitName() == "npc_dota_storegga_rock" or neutral:GetUnitName() == "npc_dota_storegga_rock2" or neutral:GetUnitName() == "npc_dota_storegga_rock3" ) then
	...
	if hHero.DeathZone then
		...
		if hHero.DeathZone.szName == "desert_start" then
			for _,VIP in pairs( hHero.DeathZone.VIPsAlive ) do
				if VIP and VIP:GetUnitName() == "npc_dota_friendly_bristleback_son" then
					vPos = VIP:GetOrigin()
				end
			end
		end
	...
	if #PlayerIDs == 1 then
		...
		for i=#PlayerIDs,1,-1 do
			for _,Relic in pairs( self.RelicsFound ) do
				if Relic and Relic["PlayerID"] == PlayerIDs[i] then
...
function CDungeon:OnCheckpointTouched( playerHero, szNewCheckpointName  )
	...
	if hBuilding then
		...
		for _,zone in pairs( self.Zones ) do
			if zone and zone:ContainsUnit( playerHero ) and zone:GetCheckpoint() ~= hBuilding then
...
function CDungeon:OnZoneActivated( Zone )
	...
	if Zone.szName == "forest_holdout" then
		...
		for _,Tower in pairs( hTowers ) do
			if Tower and Tower:GetUnitName() == "npc_dota_holdout_tower" then
	...
	if GetMapName() == "ep_2" and Zone.szName == "ice_lake" then
		...
		for _,Tower in pairs( hTowers ) do
			if Tower and Tower:GetUnitName() == "npc_dota_holdout_tower_snow" then
				Tower:SetSkin( 2 )
			end
		end
	end
...
function CDungeon:OnQuestStarted( zone, quest )
	...
	if quest.Completion.Type == QUEST_EVENT_ON_DIALOG or quest.Completion.Type == QUEST_EVENT_ON_DIALOG_ALL_CONFIRMED then
		...
		for _,DialogEnt in pairs ( hDialogEntities ) do
			if DialogEnt  and DialogEnt:GetUnitName() == quest.Completion.szNPCName and DialogEnt:FindModifierByName( "modifier_npc_dialog_notify" ) == nil then
...
function CDungeon:OnQuestCompleted( questZone, quest )
	...
	if quest.bCompleted == true then
		...
		if quest.Completion.Type == QUEST_EVENT_ON_DIALOG or quest.Completion.Type == QUEST_EVENT_ON_DIALOG_ALL_CONFIRMED then
			local hDialogEntities = FindUnitsInRadius( DOTA_TEAM_GOODGUYS, Vector( 0, 0, 0 ), nil, FIND_UNITS_EVERYWHERE, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_ALL, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, 0, false )
			for _,DialogEnt in pairs ( hDialogEntities ) do
				if DialogEnt  and DialogEnt:GetUnitName() == quest.Completion.szNPCName and DialogEnt:FindModifierByName( "modifier_npc_dialog_notify" ) then
...
function CDungeon:OnDialogBegin( hPlayerHero, hDialogEnt )
	...
	if Dialog.szRequireQuestActive then
	...
	netTable["JournalEntry"] = hDialogEnt:FindAbilityByName( "ability_journal_note" )
	netTable["WardenNote"] = hDialogEnt:FindAbilityByName( "ability_warden_note" )
	...
	if hDialogEnt:FindAbilityByName( "ability_journal_note" ) then
		local szJournalNumber = string.match( Dialog.szText, "chef_journal_(%d+)" )
		if szJournalNumber then
	...
	if hDialogEnt:FindAbilityByName( "ability_warden_note" ) then
		local szWardenNoteNumber = string.match( Dialog.szText, "warden_log_(%d+)" )
		if szWardenNoteNumber then
...
function CDungeon:OnDialogEnded( eventSourceIndex, data )
	...
	if hDialogEnt and nDialogLine then
		...
		if Dialog then
			...
			if Dialog.SpawnVIP then
				local hEnt = Entities:FindByName(nil, Dialog.SpawnPointVIP)
				if hEnt then
					local VIP = CreateUnitByName(Dialog.SpawnVIP, hEnt:GetOrigin(), true, nil, nil, DOTA_TEAM_GOODGUYS)
					if Dialog.SpawnVIP == "npc_dota_tavern_rylai" then
						VIP:SetHealth(VIP:GetMaxHealth()/99)
						VIP:SetInitialGoalEntity(Entities:FindByName(nil, "tavern_holdout_goal_rylai_vip1"))
						VIP:MoveToPositionAggressive(Entities:FindByName(nil, "tavern_holdout_goal_rylai_vip1"):GetAbsOrigin())
						Timers:CreateTimer(6.0, function()
							OpenDoor("door_tavern", true)
							VIP:SetInitialGoalEntity(Entities:FindByName(nil, "tavern_holdout_goal_rylai_vip2"))
							VIP:MoveToPositionAggressive(Entities:FindByName(nil, "tavern_holdout_goal_rylai_vip2"):GetAbsOrigin())
						end)
					end
					if Dialog.CameraFocusUnit then
						local Duration = Dialog.CameraFocusDuration
						for _, hero in pairs(HeroList:GetAllHeroes()) do
							hero:AddNewModifier(hero, nil, "modifier_command_restricted", {duration = Duration})
						end
						if Duration then
							for _, hero in pairs(HeroList:GetAllHeroes()) do
								PlayerResource:SetCameraTarget(hero:GetPlayerID(), VIP)
							...
							print("Please set a duration for the camera focus.")
						end
					end
				else
...
function CDungeon:OnBossFightIntro( hBoss )
	...
	if hBoss:FindModifierByName( "modifier_temple_guardian_statue" ) then
...
function CDungeon:OnScrollClicked( eventSourceIndex, data )
	...
	if hPlayerHero and hPlayerHero:FindModifierByName( "modifier_pre_teleport" ) then
...
function CDungeon:OnRelicClaimed( eventSourceIndex, data )
	...
	if nPlayerID and szClaimedRelicName then
		...
		if relicTable then
			...
			for _,Relic in pairs( relicTable ) do
				...
				if Relic and Relic["RelicName"] == szClaimedRelicName then
					local Hero = PlayerResource:GetSelectedHeroEntity( nPlayerID )
					if Hero then
						local nArtifactCurrencyCost = Relic["DungeonCurrencyCost"]
						local nBalance = 0
						for _,Currency in pairs ( self.ArtifactCurrency ) do
							if Currency and Currency["PlayerID"] == nPlayerID then
						...
						if nBalance < nArtifactCurrencyCost and self.bUseArtifactCurrency == true then
							...
							for _,PlayerArtifactsPurchased in pairs ( self.ArtifactsPurchased ) do
								if PlayerArtifactsPurchased and PlayerArtifactsPurchased["PlayerID"] == nPlayerID then
...
function CDungeon:OnNonPlayerUsedAbility( event )
	...
	if szAbilityName and hCaster and hCaster.bBoss == true and GameRules:GetGameTime() > hCaster.flSpeechCooldown then
