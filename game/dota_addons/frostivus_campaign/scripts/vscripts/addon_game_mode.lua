...
if GetMapName() == "frostivus_2017" then
	require("zones/dialog_frostivus_2017")
end
...
function CDungeon:InitGameMode()
	...
	if GameRules:GetGameModeEntity().SetCustomTerrainWeatherEffect then
	...
	Convars:RegisterCommand( "open_door", function(...) return OpenDoor("door_tavern", true) end, "Test a zone.", FCVAR_CHEAT )
	Convars:RegisterCommand( "close_door", function(...) return OpenDoor("door_tavern", false) end, "Test a zone.", FCVAR_CHEAT )
...
function CDungeon:OnThink()
	if GameRules:State_Get() == DOTA_GAMERULES_STATE_GAME_IN_PROGRESS then
		if self.flVictoryTime and GameRules:GetGameTime() > self.flVictoryTime then
...
function CDungeon:GetZoneByName( szZoneName )
	for _,zone in pairs( self.Zones ) do
		if zone and zone.szName == szZoneName then
			return zone
		end
	end
end
...
function CDungeon:CheckForDefeat()
	...
	for _,Hero in pairs ( Heroes ) do
		if Hero and Hero:HasOwnerAbandoned() == false and Hero:IsRealHero() and Hero:GetTeamNumber() == DOTA_TEAM_GOODGUYS and ( Hero:IsAlive() or Hero:IsReincarnating() or Hero.nRespawnsRemaining >= nBUYBACK_COST )  then
...
function CDungeon:ThinkLootExpire()
	...
	for _,item in pairs( Entities:FindAllByClassname( "dota_item_drop" ) ) do
		...
		if containedItem and not CDungeon:IsKeyItem(containedItem) and containedItem:GetAbilityName() ~= "item_life_rune" and containedItem.bIsRelic ~= true then
...
function CDungeon:GetDialog( hDialogEnt )
	...
	if hDialogEnt.nCurrentLine == nil then
 	...
 	if Dialog[hDialogEnt.nCurrentLine] and Dialog[hDialogEnt.nCurrentLine].szAdvanceQuestActive then
...
function CDungeon:IsQuestActive( szQuestName )
	for _,zone in pairs( self.Zones ) do
		if zone and zone:IsQuestActive( szQuestName ) == true then
			return true
		end
	end
...
function CDungeon:IsQuestComplete( szQuestName )
	for _,zone in pairs( self.Zones ) do
		if zone and zone:IsQuestComplete( szQuestName ) == true then
			return true
		end
	end
