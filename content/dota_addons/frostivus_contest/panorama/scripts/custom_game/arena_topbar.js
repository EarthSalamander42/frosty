...
function RadiantProgressUpdate() {
	var progress = CustomNetTables.GetTableValue("game_options", "radiant");
	if (progress.cp_score >= 0 ) {
		$('#RadiantProgressBar').value = progress.cp_score;
		$('#RadiantTakenProgressBar').value = progress.cp_score;
	} else {
		$('#RadiantProgressBar').value = (-1) * progress.cp_score;
		$('#RadiantTakenProgressBar').value = (-1) * progress.cp_score;
	}
}

// Updates the Dire contest point's progress bar
function DireProgressUpdate() {
	var progress = CustomNetTables.GetTableValue("game_options", "dire");
	if (progress.cp_score >= 0 ) {
		$('#DireProgressBar').value = progress.cp_score;
		$('#DireTakenProgressBar').value = progress.cp_score;
	} else {
		$('#DireProgressBar').value = (-1) * progress.cp_score;
		$('#DireTakenProgressBar').value = (-1) * progress.cp_score;
	}
}
